...
const bool m_bShowReport = true;	//!< 是否显示Report
...
void CGbl::GetSerialCount(std::list<UINT>& ltComID)
{
	...
	{ 
		...
		{ 
			...
			strComm.Delete(0,3);
			ltComID.push_back(atoi(strComm));
		} 
		RegCloseKey(hKey); 
	} 
}
...
//!< Crc校验
byte CGbl::BCC(unsigned char* puchMsg, unsigned short usDataLen)/* message to calculate CRC upon */
{
	byte cVal = 0;
...
//!< Crc校验
unsigned short CGbl::CRC16(unsigned char* puchMsg, unsigned short usDataLen)/* message to calculate CRC upon */
