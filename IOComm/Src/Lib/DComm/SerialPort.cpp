...
CSerialPort::CSerialPort()
{
	...
	m_uiToRead=SERIAL_INBUF_SIZE_BYTE;
	m_uiIntervalTimeOut=100;
}
...
CSerialPort::~CSerialPort()
{
	if (m_hThread) {
		ClosePort();
		DWORD dwIndex=::WaitForSingleObject(m_hThread,500);
...
// 	sprintf(szBaud, "baud=%d parity=%c data=%d stop=%d", baud, parity, databits, stopbits);
	...
	m_CommTimeouts.ReadIntervalTimeout = m_uiIntervalTimeOut;
	...
	{						   
		...
		CString strErrorID;
		strErrorID.Format("SetCommTimeouts()  错误编号:%d\r\n%X", nErrorID, nErrorID);
		ProcessErrorMessage(strErrorID.GetBuffer());
	}
...
//
unsigned int WINAPI CSerialPort::CommThread(LPVOID pParam)
{
...
{
	unsigned nThreadID;
	m_hThread=(HANDLE)::_beginthreadex(NULL,0,CommThread,(void*)this,0,&nThreadID);
	if (!m_hThread) {
		return FALSE;
	}
// 	if (!(m_Thread = AfxBeginThread(CommThread, this,THREAD_PRIORITY_HIGHEST)))
// 		return FALSE;
...
//		TRACE("发送%d个字节(%d)->",port->m_nWriteSize,::GetTickCount());
...
void CSerialPort::ReceiveChar(CSerialPort* port, COMSTAT comstat)
{
	...
	DWORD dwError = 0;
	DWORD BytesRead = 0;
	unsigned char RXBuff[SERIAL_INBUF_SIZE_BYTE];

	while(1/*g_bPortstate!=FALSE*/) 
	{ 
		...
		if (!bRead)
		{
			...
			bResult = GetOverlappedResult(port->m_hComm,	// Handle to COMM port 
										  ...
										  &BytesRead,		// Stores number of bytes read
										  TRUE); 			// Wait flag
//			TRACE("接受%d个字节(%d)->",BytesRead,::GetTickCount());
			// deal with the error code 
			if (!bResult)  
			{
...
{
	BOOL brst=SetEvent(m_hShutdownEvent);
	while (m_hThread) {
//		ClosePort();
		DWORD dwIndex=::WaitForSingleObject(m_hThread,500);
		if (dwIndex == WAIT_TIMEOUT) {
 			BOOL bl=TerminateThread(m_hThread,0);
			if(!bl)
			{
				DWORD dwrst=GetLastError();
				continue;
...
void	CSerialPort::SetRThreshold(UINT uiLen)
{
	//m_uiToRead=uiLen;;
}
UINT	CSerialPort::GetRThreshold()
{
	return m_uiToRead;
}
void	CSerialPort::SetInterval(UINT uiVal)
{
	m_uiIntervalTimeOut=uiVal;
}