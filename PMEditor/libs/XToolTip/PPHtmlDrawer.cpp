		...
		if (!sText.IsEmpty())
		{
			...
			while (!sText.IsEmpty())
			{
				...
				if (m_nMaxWidth && ((nMaxSize - sz.cx) < 0) && nTextWrapWidth)
				{
					...
					//!< Ії·ЦВ·ѕ¶І»ДЬМшіцBUG УЙjayniРЮёД
					//sTemp = GetWordWrap(sText, nTextWrapWidth, nRealSize);
					int nCut = min(nTextWrapWidth/7,sText.GetLength());
					int nNum = 0;
					while ( sTemp.GetAt(nCut-1)<0 )
					{
						nCut -= 1;
						nNum++;
					}
					nCut += nNum/2*2; 
					sTemp = sText.Left(nCut);
					sText = sText.Right(sText.GetLength()-nCut);
					//!< РЮёДНк±П
					sz.cx = nRealSize;
				}
				else
