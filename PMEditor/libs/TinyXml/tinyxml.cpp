...
TiXmlNode* TiXmlNode::AddComment(const std::string& value)
{
...
TiXmlText* TiXmlElement::AddTiXmlText(const std::string& name)
{
...
TiXmlElement* TiXmlElement::AddTiXmlChild(const std::string& name)
{
...
int TiXmlElement::GetChildSize(const std::string& name)
{
	int num=0;
	for(TiXmlNode* node=firstChild;node;node=node->next)
...
int TiXmlElement::GetAttributeSize()
{
	return attributeSet.GetSize();
}
...
bool TiXmlDocument::LoadMemory(char* buf, long length)
{
	// Delete the existing data:
	Clear();
	location.Clear();
...
char* TiXmlDocument::SaveMemory(long& length)
{
	StringToBuffer object(value);
	if (!object.buffer)				return NULL;
	if (!SaveFile(object.buffer))	return NULL;

	FILE* fp=fopen(object.buffer, "rb");
	if (!fp)						return NULL;
	fseek(fp, 0, SEEK_END);
	length = ftell(fp);
	fseek(fp, 0, SEEK_SET);
...
void TiXmlAttribute::Print( FILE* cfile, int  ) const
{
	TIXML_STRING n, v;
