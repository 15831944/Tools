...
{
public:
	std::vector<std::shared_ptr<CCamera> > m_vtCam;			//!< 摄像头数组
	std::list<std::shared_ptr<CCamera> > m_ltItemClipBoard;	//!< 变量的剪贴板
...
public:
	...
	std::shared_ptr<CCamera> GetCam(ULONG id);				//!< 获得id号变量
	std::shared_ptr<CCamera> GetFirstItem();					//!< 获得第一个变量
	std::shared_ptr<CCamera> GetLastItem();					//!< 获得最后一个变量
	bool FindItem(ULONG id);									//!< 查找编号为id的变量
	bool AddItem(std::shared_ptr<CCamera> item);				//!< 添加变量,这时变量还没有ID，组内变量不能重名
