...
CString CMyMD5::MD5(const CString& strFilePath)
{
...
CString CMyMD5::MD5(CFile& File)
{
	try
	{
		CMyMD5 MD5_VC;     //MD5_VC object 
...
CString CMyMD5::MD5(BYTE* pBuf, UINT nLength)
{
	//entry invariants
...
DWORD CMyMD5::RotateLeft(DWORD x, int n)
{
...
void CMyMD5::FF( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD F = (B & C) | (~B & D);
	A += F + X + T;
	A = RotateLeft(A, S);
...
void CMyMD5::GG( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD G = (B & D) | (C & ~D);
	A += G + X + T;
	A = RotateLeft(A, S);
...
void CMyMD5::HH( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD H = (B ^ C ^ D);
	A += H + X + T;
	A = RotateLeft(A, S);
...
void CMyMD5::II( DWORD& A, DWORD B, DWORD C, DWORD D, DWORD X, DWORD S, DWORD T)
{
	DWORD I = (C ^ (B | ~D));
	A += I + X + T;
	A = RotateLeft(A, S);
...
void CMyMD5::Transform(BYTE Block[64])
{
...
void CMyMD5::ByteToDWord(DWORD* Output, BYTE* Input, UINT nLength)
{
	//entry invariants
	ASSERT( nLength % 4 == 0 );
...
void CMyMD5::DWordToByte(BYTE* Output, DWORD* Input, UINT nLength )
{
	//entry invariants
	ASSERT( nLength % 4 == 0 );
...
void CMyMD5::Update( BYTE* Input, ULONG nInputLen )
{
